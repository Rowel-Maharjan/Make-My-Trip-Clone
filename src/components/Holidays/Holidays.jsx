          ...
          <div className="flex gap-28 mr-12">
            <ul className="flex gap-1 flex-col text-[#4a4a4a]">
              {places.slice(0, 5).map((place, index) => (
                <li key={index}>
                  <a className="cursor-pointer">{place}</a>
                </li>
              ))}
            </ul>
            <ul className="flex gap-1 flex-col text-[#4a4a4a]">
    ...
    <>
      <div className="w-[1200px] relative bg-white pb-7  -top-12 rounded-xl">
        <div className="w-full h-[100px] mb-3 bg-[#eaf5ff]">
          <div className="grid grid-cols-6 h-[100px]">
            {holidayInfo.map((info) => (
              <div
                key={info.id}
                onClick={() => setholidayActiveTab(info.value)}
                defaultValue={holidayActiveTab}
                className="flex relative items-end justify-center border-r border-r-[#d8d8d8] cursor-pointer"
              >
                ...
                {holidayActiveTab === info.value && (
                  <div className="absolute w-full h-1 bg-[#008cff] left-0 bottom-0">
                    <div
        ...
        <div className="px-5">
            ...
            <>
              ...
              <p className="flex justify-center relative">
                <button
                  ...
                  className="absolute -bottom-7 px-5 py-2 -mb-5 inline-block bg-gradient-to-r min-w-[216px] from-[#53b2fe] to-[#065af3] rounded-full text-white text-2xl font-bold "
                >
                  SEARCH
                </button>
              </p>
          ...
          {holidayActiveTab === "Book@1" && (
            <HoldiayContent
              title={"Book@1"}
              places={HolidaysPlaceItems}
              PicturePlace={HolidaysPlace}
            />
          )}

          {holidayActiveTab === "Cruise" && (
            <HoldiayContent
              title={"MMT Holidays Cruise Packages"}
              places={CruisePlaceItems}
              PicturePlace={CruisePlace}
            />
          )}

          {holidayActiveTab === "Honeymoon" && (
            <HoldiayContent
              title={"MakeMyTrip Honeymoon Packages"}
              places={HoneyMoonItems}
              PicturePlace={HoneyMoonPlace}
            />
          )}

          {holidayActiveTab === "Air Nepal Express Holidays" && (
            <HoldiayContent
              title={"Domestic & International"}
              places={AirNepalExpress}
            />
          )}

          {holidayActiveTab === "Luxe Holidays" && (
            <HoldiayContent
              title={"MMT Holidays Premium Packages"}
              places={LuxeItems}
              PicturePlace={LuxePlace}
            />
          )}
        </div>
      </div>
