...
const Cabs = () => {
  ...
  const onSubmit = () => {
    if (cabActiveTab === "Outstation One-Way") console.log(formDataOneWay);
    ...
    else console.log(formDataHourlyRental);
  };
  return (
    <>
      <div className="w-[1200px] relative bg-white pt-16 pb-10 px-5 -top-12 rounded-xl mb-11 shadow-2xl">
        ...
        <div className="flex justify-between gap-10">
          <RadioGroup className="flex gap-4 mb-2" defaultValue={cabActiveTab}>
            {cabsInfo.map((cabInfo) => (
              <Label
                ...
                key={cabInfo.id}
                onClick={() => {
                  setcabActiveTab(cabInfo.value);
                }}
                className={`flex cursor-pointer items-center p-1 ${
                  cabInfo.value === cabActiveTab
              ...
              >
                ...
                <Label
                  className={`cursor-pointer pl-2 font-normal text-sm text-[#4a4a4a] ${
                    cabInfo.value === cabActiveTab ? "font-bold text-black" : ""
                  }`}
                  htmlFor={cabInfo.id}
                >
                  {cabInfo.value}
          ...
          <div className="text-[#4a4a4a] font-normal text-sm">
            Online Cab Booking
          </div>
        </div>
        ...
        {cabActiveTab === "Outstation One-Way" && (
          <>
            <div className="w-full border relative rounded-lg mb-5 flex border-[#e7e7e7]">
              {cabsOneWay.map((cabValue) => (
                <Form
                  onChangeHandler={onChangeHandlerOneWay}
                  key={cabValue.title}
                  formValue={cabValue}
                />
              ))}
            </div>
            <div className="text-[#008cff] font-bold cursor-pointer ml-5">
              + Add Stops
            </div>
          </>
        )}
        ...
        {cabActiveTab === "Outstation Round-Trip" && (
          <>
            <div className="w-full border rounded-lg mb-5 flex border-[#e7e7e7]">
              {cabsRoundTrip.map((cabValue) => (
                <Form
                  onChangeHandler={onChangeHandlerRoundTrip}
                  key={cabValue.title}
                  formValue={cabValue}
                />
              ))}
        ...
        {cabActiveTab === "Airport Transfers" && (
          <>
            <div className="w-full border rounded-lg mb-5 flex border-[#e7e7e7]">
              {cabsAirportTransfer.map((cabValue) => (
                <Form
                  onChangeHandler={onChangeHandlerAirportTransfer}
                  key={cabValue.title}
                  formValue={cabValue}
                />
              ))}
        ...
        {cabActiveTab === "Hourly Rentals" && (
          <>
            <div className="w-full border rounded-lg mb-5 flex border-[#e7e7e7]">
              {cabsHourlyRentals.map((cabValue) => (
                <Form
                  onChangeHandler={onChangeHandlerHourlyRental}
                  key={cabValue.title}
                  formValue={cabValue}
                />
              ))}
